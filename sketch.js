...
const IDLE_FRAME_WIDTH_AVG = IDLE_SPRITE_TOTAL_WIDTH / 8; // 63.75
...
let centerCharX, leftCharX, farLeftCharX; // 中間和左邊角色的固定位置 (不可移動)
...
const NEW_CHAR_SPRITE_TOTAL_WIDTH = 472;
const NEW_CHAR_FRAME_WIDTH = 59; // 472 / 8 = 59
...
// --- 新增角色 3 (最左邊 G-stand) ---
let fourthCharSpriteSheet;
let fourthCharAnimation = [];
const FOURTH_CHAR_SPRITE_TOTAL_WIDTH = 643;
const FOURTH_CHAR_FRAME_WIDTH_AVG = FOURTH_CHAR_SPRITE_TOTAL_WIDTH / 9; // ~71.44
const FOURTH_CHAR_FRAME_HEIGHT = 93;
const NUM_FOURTH_CHAR_FRAMES = 9;
const FOURTH_CHAR_ANIMATION_SPEED = 12; 
...
function preload() {
  ...
  fourthCharSpriteSheet = loadImage('../4(G-stand)/All.png');
...
function setup() {
  ...
  // 原人物 G (右) 動畫切割 (站立 - 510x95)
  let currentX_Idle = 0;
  for (let i = 0; i < NUM_IDLE_FRAMES; i++) {
    const frameW = (i < 6) ? 64 : 63; // 6*64 + 2*63 = 510
    let frame = idleSpriteSheet.get(currentX_Idle, 0, frameW, IDLE_FRAME_HEIGHT);
    idleAnimation.push(frame);
    currentX_Idle += frameW;
  }
  ...
  // 新增角色 3 (最左邊 G-stand) 動畫切割
  let currentX_3 = 0;
  for (let i = 0; i < NUM_FOURTH_CHAR_FRAMES; i++) {
    const frameW = (i < 5) ? 71 : 72; // 5*71 + 4*72 = 643
    let frame = fourthCharSpriteSheet.get(currentX_3, 0, frameW, FOURTH_CHAR_FRAME_HEIGHT);
    fourthCharAnimation.push(frame);
    currentX_3 += frameW;
  }
...
function recalculateCenterPositions() {
  const totalWidth = 
    FOURTH_CHAR_FRAME_WIDTH_AVG + CHARACTER_SPACING + 
    THIRD_CHAR_FRAME_WIDTH_AVG + CHARACTER_SPACING + 
    NEW_CHAR_FRAME_WIDTH + CHARACTER_SPACING + 
    IDLE_FRAME_WIDTH_AVG; 
  
  const startX = (width / 2) - (totalWidth / 2); 
  
  farLeftCharX = startX + (FOURTH_CHAR_FRAME_WIDTH_AVG / 2);
  leftCharX = startX + FOURTH_CHAR_FRAME_WIDTH_AVG + CHARACTER_SPACING + (THIRD_CHAR_FRAME_WIDTH_AVG / 2);
  centerCharX = startX + FOURTH_CHAR_FRAME_WIDTH_AVG + CHARACTER_SPACING + THIRD_CHAR_FRAME_WIDTH_AVG + CHARACTER_SPACING + (NEW_CHAR_FRAME_WIDTH / 2);

  x = startX + 
      FOURTH_CHAR_FRAME_WIDTH_AVG + CHARACTER_SPACING + 
      THIRD_CHAR_FRAME_WIDTH_AVG + CHARACTER_SPACING + 
      NEW_CHAR_FRAME_WIDTH + CHARACTER_SPACING + 
      (IDLE_FRAME_WIDTH_AVG / 2);
}
...
function drawCharacter() {
  ...
  let frameCountSpeed = IDLE_ANIMATION_SPEED;
  let numFrames = NUM_IDLE_FRAMES;
...
/**
 * 繪製第四個角色 G-stand (最左) (使用固定的 farLeftCharX)
 */
function drawFourthCharacter() {
  const charX = farLeftCharX;
  const charY = y; 

  const currentFrameIndex = floor(frameCount / FOURTH_CHAR_ANIMATION_SPEED) % NUM_FOURTH_CHAR_FRAMES;
  
  image(
    fourthCharAnimation[currentFrameIndex], 
    charX, 
    charY
  );
}
